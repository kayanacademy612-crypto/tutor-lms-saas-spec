...
---
Task ID: P6-A1
Agent: Backend Reports+AI+Migration Models
Task: Create reportsai.go models, collections, events

Work Log:
- Read `/home/z/my-project/repos/lastsaas/backend/internal/models/proengagement.go` (218 lines) end-to-end as the canonical model pattern: package `models`, `import (time; primitive)`, header comment block describing the phase feature set, type-then-constants grouping, `TenantID primitive.ObjectID \`json:"tenantId" bson:"tenantId" validate:"required"\`` on every struct, `time.Time` for timestamps, gofmt-aligned struct field tags with inline comments for enums.
- Read `/home/z/my-project/repos/lastsaas/backend/internal/db/proengagement_collections.go` (69 lines) to confirm the collection accessor pattern: package `db`, single `import "go.mongodb.org/mongo-driver/mongo"`, one method per collection named after the plural resource, body is `return m.Database.Collection("lms_<resource_plural>")`, header comment block + per-method doc comment.
- Read `/home/z/my-project/repos/lastsaas/backend/internal/events/proengagement_events.go` (46 lines) to confirm the event-constant pattern: package `events`, dotted-lowercase `EventType` values grouped by resource in a single `const (...)` block, header comment documenting any overlap with `lms_events.go` constants that are intentionally NOT re-declared.
- Read `/home/z/my-project/repos/lastsaas/backend/internal/models/lms.go` lines 870-929 to inspect the existing `MigrationStatus` enum + `Migration` struct (Pending/Running/Complete/Failed, Source/TotalItems/DoneItems/FailedItems/LogURL/Error/Config/StartedAt/FinishedAt/CreatedAt/UpdatedAt). Confirmed the new `MigrationJob` struct extends this with per-entity counters (courses/lessons/quizzes/students), a richer `MigrationJobStatus` enum that adds `cancelled`, and a `StartedBy` audit field.
- Read `/home/z/my-project/repos/lastsaas/backend/internal/events/lms_events.go` (135 lines) and confirmed the four legacy migration event constants already declared in the `// --- Migrations ---` group: `EventMigrationStarted` ("migration.started"), `EventMigrationProgress` ("migration.progress"), `EventMigrationComplete` ("migration.complete"), `EventMigrationFailed` ("migration.failed"). The Phase 6 migration events use the distinct `migration.job_*` / `migration.batch_completed` namespaces and do NOT collide with these — declared in the new file with a header comment documenting the overlap.
- Read `/home/z/my-project/repos/lastsaas/backend/internal/db/mongodb.go` lines 1-536 to confirm the `ensureIndexes()` pattern: a single `indexes := []struct{collection string; models []mongo.IndexModel}{...}` slice, unique indexes set via `Options: options.Index().SetUnique(true)`, sparse indexes via `SetSparse(true)`, and a `criticalCollections` map controlling whether index failure is fatal. Inserted the 8 new Phase 6 collection blocks immediately after the `lms_legal_consents` block (Phase 5) and before the closing `}` of the slice.
- Created `/home/z/my-project/repos/lastsaas/backend/internal/models/reportsai.go` (8 structs + 4 enums): `ReportType` + `ReportSnapshot` + `SavedReport` (Reports); `AIConversation` + `AIMessageRole` + `AIMessage` + `AIUsageStats` (TutorAI); `MigrationPlatform` + `MigrationJobStatus` + `MigrationJob` + `MigrationLog` + `MigrationMapping` (Migration). Every struct carries `TenantID primitive.ObjectID \`... validate:"required"\``. Header comment explains the relationship between the new `MigrationJob` and the existing `Migration` struct in lms.go.
- Created `/home/z/my-project/repos/lastsaas/backend/internal/db/reportsai_collections.go` (8 accessors): `ReportSnapshots`, `SavedReports`, `AIConversations`, `AIMessages`, `AIUsageStats`, `MigrationJobs`, `MigrationLogs`, `MigrationMappings` — each returning `m.Database.Collection("lms_<resource_plural>")`.
- Created `/home/z/my-project/repos/lastsaas/backend/internal/events/reportsai_events.go` (11 event constants in one `const (...)` block): `EventReportGenerated`/`EventReportSaved`/`EventReportScheduled` (Reports); `EventAIConversationCreated`/`EventAIMessageSent`/`EventAIResponseReceived`/`EventAIUsageLimitReached` (TutorAI); `EventMigrationJobStarted`/`EventMigrationJobCompleted`/`EventMigrationJobFailed`/`EventMigrationBatchCompleted` (Migration). Header comment explicitly lists the four legacy migration events in lms_events.go that are intentionally NOT re-declared.
- Modified `/home/z/my-project/repos/lastsaas/backend/internal/db/mongodb.go` `ensureIndexes()` to append 8 new collection blocks with 8 indexes total: `lms_report_snapshots` `{tenantId, reportType, createdAt: -1}`; `lms_saved_reports` `{tenantId, createdBy}`; `lms_ai_conversations` `{tenantId, userId, updatedAt: -1}`; `lms_ai_messages` `{tenantId, conversationId, createdAt: 1}`; `lms_ai_usage_stats` `{tenantId, userId, date}` **unique** (one row per user per day, upserted by the metering job); `lms_migration_jobs` `{tenantId, status, createdAt: -1}`; `lms_migration_logs` `{tenantId, jobId, createdAt: 1}`; `lms_migration_mappings` `{tenantId, jobId, entityType, sourceId}` **unique** (idempotent re-runs).
- Ran `gofmt -w` on all 4 files (3 new + 1 modified) to align struct field tags and index model entries with the existing codebase style.
- Verified with `go build ./...` from `/home/z/my-project/repos/lastsaas/backend` — exit code 0, zero output.
- Verified with `go vet ./internal/models/... ./internal/db/... ./internal/events/...` — exit code 0, zero diagnostics on the touched packages. (Pre-existing vet warnings in `internal/api/handlers/tenant_test.go` are unrelated to Phase 6.)

Stage Summary:
- Created `internal/models/reportsai.go` with 8 structs (`ReportSnapshot`, `SavedReport`, `AIConversation`, `AIMessage`, `AIUsageStats`, `MigrationJob`, `MigrationLog`, `MigrationMapping`) + 4 enums (`ReportType`, `AIMessageRole`, `MigrationPlatform`, `MigrationJobStatus`).
- Created `internal/db/reportsai_collections.go` with 8 collection accessors.
- Created `internal/events/reportsai_events.go` with 11 event constants (3 reports + 4 AI + 4 migration).
- Added 8 indexes across 8 new collections to `mongodb.go` `ensureIndexes()` (2 unique: `lms_ai_usage_stats`, `lms_migration_mappings`).
- Build status: PASS (`go build ./...` exits 0).
- Vet status: PASS on `internal/models/...`, `internal/db/...`, `internal/events/...`.
- Duplicate events found: 4 legacy migration events in `lms_events.go` (`EventMigrationStarted`/`Progress`/`Complete`/`Failed` using the `migration.started/.progress/.complete/.failed` values). Phase 6 migration events use distinct names (`EventMigrationJob*`, `EventMigrationBatchCompleted`) and distinct values (`migration.job_*`, `migration.batch_completed`) — no collision. The 4 legacy constants are intentionally NOT re-declared; a header comment in `reportsai_events.go` documents them.

Notes for downstream agents:
- The existing `Migration` struct + `MigrationStatus` enum in `lms.go` (lines 873-899) and the 4 legacy migration event constants in `lms_events.go` (lines 119-123) are kept for back-compat with the Phase-1 importer. New Phase 6 migration handlers should prefer the richer `MigrationJob` / `MigrationJobStatus` / `EventMigrationJob*` set defined here.
- `MigrationMapping` has a unique compound index on `(tenantId, jobId, entityType, sourceId)`. Re-running a MigrationJob must therefore go through the same `jobId` to be idempotent (a re-run with a new `jobId` will insert new mappings rather than reuse old ones). If a re-run-by-new-job semantics is desired, change the unique index to drop `jobId` or relax it to a non-unique index.
- `AIUsageStats.Date` is documented as "YYYY-MM-DD stored at midnight UTC"; the unique index `{tenantId, userId, date}` relies on the metering job normalising the timestamp to midnight before the upsert.

---
Task ID: P6-A2
Agent: Frontend Reports+AI+Migration API + Hooks
Task: Add Phase 6 types, API, hooks

Work Log:
- Read `/home/z/my-project/worklog.md` P5-A2 section to confirm the established frontend API/hooks contract: `UseLmsQueryResult<T> = { data, loading, error, refetch }`, `UseLmsMutationResult<T,V> = { data, loading, error, mutate, reset }`, plain `useState`+`useEffect` (no React Query), `useIsMounted` + per-fetch token ref guard against setState-after-unmount and stale-response-overwrite, list endpoints normalised to `T[]` via the local `toList()` helper, mutations that take a resource id receive it at `mutate(...)` time (via the vars object) so a single hook instance can act on any row.
- Read `src/hooks/useProEngagement.ts` (1029 lines) end-to-end as the canonical Phase 5 hook pattern: `argsKey(args)` helper for stable fetch-key deps, id-skipping query hooks (`Boolean(id)` initial loading + early-return when `!id`), `useEffect` keyed on `argsKey([...])` with the `eslint-disable react-hooks/exhaustive-deps` carve-out, mutation hooks structured as `mutate(vars) → setLoading(true) → try { result = await lmsApi.X.Y(...) } catch { setError } finally { setLoading(false) }` with a `reset()` that clears `data/loading/error`.
- Read `src/services/lms-api.ts` to confirm the resource-group pattern: `export const fooApi = { list(), get(), create(), update(), remove() }` returning `Promise<T>` via the `unwrap()` helper, `toQuery(params)` for `ListParams` → `URLSearchParams` conversion (returns `undefined` when empty so axios skips the `?`), and a single barrel `export const lmsApi = { course: courseApi, ..., phase5 groups }` at the bottom of the file.
- Read `src/types/lms.ts` lines 925-955 to inspect the existing Phase-1 `Migration`/`MigrationStatus`/`MigrationCreateInput` types and the existing `migrationApi` (lines 776-785 of `lms-api.ts`) that exposed only `list()` + `create()` against the legacy `Migration` model. Confirmed via repo-wide grep that no UI consumers (pages/hooks) reference these legacy migration types — only the type definition and the API definition — so replacing `migrationApi` with the Phase 6 `MigrationJob`-based surface is safe and non-breaking.
- Appended the Phase 6 type block to `src/types/lms.ts` (after the existing `ConsentInput` interface, before EOF): `ReportType` + `ReportSnapshot` + `SavedReport` + `ReportFilters` + `OverviewReport` + `SalesReport` + `EnrollmentReport` + `CompletionReport` + `CourseReport` + `StudentReport` (10 report types); `AIConversation` + `AIMessage` + `AISendMessageInput` + `AISendMessageResult` + `AIUsageStats` (5 AI types); `MigrationPlatform` + `MigrationJobStatus` + `MigrationJob` + `MigrationLog` + `CreateMigrationInput` (5 migration types). All ids/timestamps use plain `string` (not the legacy `ObjectID`/`ISODateString` aliases) to match the task spec verbatim. Total: 20 new types/enums.
- Extended `src/services/lms-api.ts` imports (alphabetised) to add the 18 new types used by the new API groups: `AIConversation`, `AIMessage`, `AISendMessageInput`, `AISendMessageResult`, `AIUsageStats`, `CompletionReport`, `CourseReport`, `CreateMigrationInput`, `EnrollmentReport`, `MigrationJob`, `MigrationLog`, `MigrationPlatform`, `OverviewReport`, `ReportFilters`, `ReportType`, `SalesReport`, `SavedReport`, `StudentReport`. Removed the now-unused `Migration` and `MigrationCreateInput` imports.
- Replaced the legacy Phase-1 `migrationApi` block (bare `list()` + `create()` on `Migration`) with the Phase 6 `migrationApi`: `list(params?) → MigrationJob[]`, `get(id) → MigrationJob`, `create(input: CreateMigrationInput) → MigrationJob`, `start(id) → MigrationJob`, `cancel(id) → MigrationJob`, `logs(id, params?) → MigrationLog[]`. All id-bearing path segments use `encodeURIComponent(id)`. Left a header comment documenting the relationship between the new `MigrationJob`-based surface and the legacy `Migration` model (whose type definitions remain in `lms.ts` for back-compat with any external consumers).
- Added `reportApi` (10 methods) and `aiApi` (7 methods) just above the barrel export, mirroring the existing resource-group style: per-method doc comment with the HTTP verb + path, single-line `unwrap(lmsAxios.<verb>(<path>, <body>))` body, and `?? undefined` normalisation of optional `params` so axios skips the `?` when the caller passes nothing. `reportApi` covers `overview`/`sales`/`enrollments`/`completion`/`courses`/`students`/`exportCsv`/`save`/`listSaved`/`deleteSaved`. `aiApi` covers `sendMessage`/`conversations`/`conversation`/`deleteConversation`/`usage`/`generateCourseOutline`/`generateQuiz`.
- Updated the `lmsApi` barrel export to add `report: reportApi` and `ai: aiApi` under a new `// Phase 6 — Reports + AI` comment. The `migration: migrationApi` entry was already present (from Phase 1) and now resolves to the new comprehensive surface — no barrel edit needed for migration.
- Created `src/hooks/useReportsAI.ts` (23 hooks total — 10 Reports, 7 AI, 6 Migration) following the `useProEngagement.ts` pattern exactly: shared `argsKey()` + `toList()` helpers at the top; query hooks structured as `useState(null/[]) + useState(true) + useState(null) + useIsMounted() + useRef(0) + useCallback(run, [deps]) + useEffect(...argsKey([params])...)`; mutation hooks structured as `useState(null) + useState(false) + useState(null) + useIsMounted() + useCallback(mutate, [isMounted]) + useCallback(reset, [])`.
  - Reports queries (6): `useOverviewReport(params?)`, `useSalesReport(params?)`, `useEnrollmentReport(params?)`, `useCompletionReport(params?)`, `useCourseReport(params?)`, `useStudentReport(params?)` — all return `UseLmsQueryResult<TReport>` and refetch on `argsKey([params])` change.
  - Reports queries (1): `useSavedReports()` — returns `UseLmsQueryResult<SavedReport[]>`, refetch on `run` identity only.
  - Reports mutations (3): `useExportReport()` (vars `{ reportType, params? }` → `{ downloadUrl }`), `useSaveReport()` (vars `{ name, reportType, config }` → `SavedReport`), `useDeleteSavedReport()` (vars `id` → `{ success }`).
  - AI queries (2): `useAIConversations()` (returns `AIConversation[]`), `useAIConversation(id?)` (returns `{ conversation, messages }`, skips fetch when `!id`).
  - AI queries (1): `useAIUsage(params?)` (returns `AIUsageStats[]`, refetch on `argsKey([params])`).
  - AI mutations (4): `useSendAIMessage()` (vars `AISendMessageInput` → `AISendMessageResult`), `useDeleteAIConversation()` (vars `id` → `{ success }`), `useGenerateCourseOutline()` (vars `{ topic, level?, lessonsCount? }` → `{ outline }`), `useGenerateQuiz()` (vars `{ lessonId?, topic, questionCount?, questionTypes? }` → `{ quiz }`).
  - Migration queries (3): `useMigrations(params?)` (returns `MigrationJob[]`), `useMigration(id?)` (returns `MigrationJob`, skips fetch when `!id`), `useMigrationLogs(id?, params?)` (returns `MigrationLog[]`, skips fetch when `!id`, refetch on `argsKey([id, params])`).
  - Migration mutations (3): `useCreateMigration()` (vars `CreateMigrationInput` → `MigrationJob`), `useStartMigration()` (vars `id` → `MigrationJob`), `useCancelMigration()` (vars `id` → `MigrationJob`).
- Ran `npx tsc --noEmit` from `/home/z/my-project/repos/tailux/tailux-main` — exit code 0, zero diagnostics. No consumers of the legacy `Migration`/`MigrationStatus`/`MigrationCreateInput` types or the old 2-method `migrationApi` exist in the repo (verified by repo-wide grep before the replacement), so the substitution is non-breaking.

Stage Summary:
- Added 20 Phase 6 types/enums (10 Reports, 5 AI, 5 Migration) to `src/types/lms.ts` — file grew from 1842 to 2117 lines.
- Extended `src/services/lms-api.ts` with 3 API groups (the `migrationApi` was replaced in place with the comprehensive Phase 6 surface; `reportApi` and `aiApi` are new): 10 report methods, 7 AI methods, 6 migration methods — file grew from 1676 to 1846 lines.
- Created `src/hooks/useReportsAI.ts` with 23 hooks (10 Reports + 7 AI + 6 Migration): 9 query hooks, 14 mutation... correction: 9 query hooks (`useOverviewReport`, `useSalesReport`, `useEnrollmentReport`, `useCompletionReport`, `useCourseReport`, `useStudentReport`, `useSavedReports`, `useAIConversations`, `useAIConversation`, `useAIUsage`, `useMigrations`, `useMigration`, `useMigrationLogs` = 13 query hooks) + 10 mutation hooks (`useExportReport`, `useSaveReport`, `useDeleteSavedReport`, `useSendAIMessage`, `useDeleteAIConversation`, `useGenerateCourseOutline`, `useGenerateQuiz`, `useCreateMigration`, `useStartMigration`, `useCancelMigration`). Total 23 hooks.
- TypeScript check: PASS (`npx tsc --noEmit` exits 0, zero diagnostics).

Notes for downstream agents:
- The legacy `Migration`/`MigrationStatus`/`MigrationCreateInput` types remain in `src/types/lms.ts` (lines 933-955) for back-compat with any external consumers but are no longer imported by `lms-api.ts` or any hook. The Phase 6 migration surface uses `MigrationJob`/`MigrationJobStatus`/`MigrationLog`/`CreateMigrationInput`/`MigrationPlatform`. If a future cleanup task wants to remove the legacy types, run a repo-wide grep first to confirm zero external imports — at the time of this writing only `lms.ts` (definition) and the Phase 6 header comment in `lms-api.ts` (doc reference) mention them.
- `useReportsAI.ts` is NOT re-exported from `src/hooks/index.ts` (mirrors the `useProEngagement.ts` convention — Phase 5/6 hooks are imported directly from their source file via `@/hooks/useReportsAI`). Pages that need these hooks should use `import { useOverviewReport, useSendAIMessage, useMigrations, ... } from "@/hooks/useReportsAI"`.
- The `reportApi` `exportCsv()` method returns a short-lived `downloadUrl` (typically a pre-signed S3 URL); callers should hand it to `window.location.href` or an `<a download>` anchor. The hook does NOT auto-trigger the download — that's the page's responsibility so it can show a "preparing export..." toast while the backend generates the file.
- `useSendAIMessage` does NOT auto-append the assistant's reply to any conversation cache — pages that show a conversation history should refetch via `useAIConversation(id).refetch()` after a successful `mutate()` (or optimistically append the user message + assistant reply to local state). The `AISendMessageResult` includes `conversationId` so the page can detect "first message in a new conversation" and redirect from the new-conversation route to the `/conversations/{id}` route.

---
Task ID: P6-A4
Agent: Backend AI Handlers
Task: Create ai_handler.go with chat, conversations, usage, course outline, quiz generation

Work Log:
- Read `/home/z/my-project/worklog.md` P6-A1 stage summary to confirm the Phase 6 model surface (`AIConversation`, `AIMessage`, `AIMessageRole`, `AIUsageStats`), the collection accessors (`db.AIConversations()`/`AIMessages()`/`AIUsageStats()`), and the four TutorAI event constants (`EventAIConversationCreated`/`EventAIMessageSent`/`EventAIResponseReceived`/`EventAIUsageLimitReached`). Also noted the P6-A2 frontend contract: `aiApi` exposes `sendMessage`/`conversations`/`conversation`/`deleteConversation`/`usage`/`generateCourseOutline`/`generateQuiz` — the backend handler names match this contract exactly.
- Read `/home/z/my-project/repos/lastsaas/backend/internal/api/handlers/proengagement_gamification.go` (lines 1-200, 735-794, 1270-1284) as the canonical handler pattern: `struct { db *db.MongoDB; emitter events.Emitter }` + `NewXHandler(database, emitter) *XHandler` constructor, `requireContext(w, r) (lmsContext, bool)` helper that reuses `getLMSContext(r)` and 401s when `ctx.UserID.IsZero()`, `h.emitter.Emit(events.Event{Type, Timestamp, Data})` for event emission, `bson.M{"tenantId": ctx.TenantID, ...}` filters on every query.
- Read `/home/z/my-project/repos/lastsaas/backend/internal/api/handlers/lms.go` lines 56-115 to confirm `getLMSContext(r)` resolves `{TenantID, UserID, IsInstructor}` from middleware context (with the dev fallback to `000000000000000000000001` tenant + `000000000000000000000002` user), and lines 4000-4014 to confirm the `parsePositiveInt(r, key, default, max)` helper signature for paginated list endpoints.
- Read `/home/z/my-project/repos/lastsaas/backend/internal/api/handlers/admin.go` lines 844-846 to confirm the `decodeJSON(r, v)` helper signature (single-line `json.NewDecoder(r.Body).Decode(v)`).
- Read `/home/z/my-project/repos/lastsaas/backend/internal/api/handlers/helpers.go` (full file, 65 lines) to confirm `respondWithJSON`/`respondWithError`/`ErrorResponse` and that no OpenAI-related helpers already exist.
- Read `/home/z/my-project/repos/lastsaas/backend/internal/models/lms.go` lines 136-156 to inspect the `Lesson` struct (`ID`, `TenantID`, `CourseID`, `Title`, `Content`) so the quiz generator can fetch lesson content for grounding when `lessonId` is supplied.
- Read `/home/z/my-project/repos/lastsaas/backend/internal/db/reportsai_collections.go` (full file, 59 lines) to confirm the `AIConversations`/`AIMessages`/`AIUsageStats` accessors return `m.Database.Collection("lms_ai_conversations"|"lms_ai_messages"|"lms_ai_usage_stats")`. Read `/home/z/my-project/repos/lastsaas/backend/internal/events/reportsai_events.go` (full file, 47 lines) to confirm the four TutorAI event constants and the `Event{Type, Timestamp, Data}` struct shape.
- Read `/home/z/my-project/repos/lastsaas/backend/internal/events/emitter.go` (full file, 53 lines) to confirm `Emitter` is `Emit(Event)` and `Event.Data` is `map[string]interface{}`.
- Checked `/home/z/my-project/repos/lastsaas/backend/go.mod` — confirmed `github.com/sashabaranov/go-openai` is NOT a dependency. Per the task spec, implemented the OpenAI passthrough directly via `net/http` (no new go.mod entries, no `go mod tidy` needed). The `callOpenAI(ctx, model, messages, maxTokens, temperature) (content string, tokensUsed int, err error)` helper POSTs to `https://api.openai.com/v1/chat/completions` with `Authorization: Bearer <OPENAI_API_KEY>`, decodes the standard `{choices:[{message:{role,content}}], usage:{prompt_tokens,completion_tokens,total_tokens}}` response, and surfaces upstream non-200 responses with the OpenAI error message body.
- Created `/home/z/my-project/repos/lastsaas/backend/internal/api/handlers/ai_handler.go` (≈1280 lines, 7 HTTP handlers + helpers + wire types):
  - **`AIHandler` struct + `NewAIHandler(database, emitter)`** — mirrors the `ProEngagementGamificationHandler` constructor pattern.
  - **`requireContext(w, r)`** — reuses `getLMSContext(r)` and 401s on missing UserID.
  - **`getOpenAIKey()`/`getOpenAIModel()`/`getOpenAIEndpoint()`** — env-backed config readers (`OPENAI_API_KEY` required; `OPENAI_MODEL` defaults to `gpt-4`; `OPENAI_ENDPOINT` defaults to the public Chat Completions URL).
  - **`requireOpenAIKey(w)`** — writes HTTP 503 with `"AI service not configured. Set OPENAI_API_KEY environment variable."` when the key is unset (no crash).
  - **`callOpenAI(ctx, model, messages, maxTokens, temperature)`** — pure `net/http` passthrough; 60s timeout via the supplied context; decodes the response and returns `(content, totalTokens, err)`.
  - **`buildSystemPrompt(ctx)`** — default TutorAI prompt or one of four action-specific prompts (`content_assist`, `course_outline`, `quiz_generation`, `tutor`).
  - **`truncateTitle(s, max)`** — rune-safe title slicer (first 50 chars + ellipsis) for new conversation titles.
  - **`normalizeToDate(t)`** — UTC midnight normaliser for the AIUsageStats partition key.
  - **`getTodayUsage(ctx, tenantID, userID)`** — returns today's `AIUsageStats` row or a zero-valued placeholder.
  - **`incrementDailyUsage(ctx, tenantID, userID, date, requests, tokens)`** — `$inc` upsert against the unique `(tenantId, userId, date)` index.
  - **`getConversationHistory(ctx, tenantID, conversationID, limit)`** — last N messages in oldest→newest order for OpenAI context.
  - **`extractJSON(s)`** — strips ```json / ``` markdown fences so generated JSON can be `json.Unmarshal`ed reliably.
  - **`truncateForPrompt(s, maxBytes)`** — rune-safe byte truncation for lesson content grounding in the quiz generator.
  - **`Chat`** — full 11-step flow: parse → pre-flight quota check → resolve/create conversation (emit `EventAIConversationCreated` when new) → save user message (emit `EventAIMessageSent`) → build OpenAI messages (system prompt + last 10 history + new user message) → call OpenAI → save assistant message (emit `EventAIResponseReceived`) → `$inc` conversation.UsageTokens → upsert AIUsageStats → compute remaining tokens → return `{conversationId, message: AIMessage, usage: {tokensUsed, remainingTokens}}`. Emits `EventAIUsageLimitReached` + 429 when the pre-flight check shows the user is already at the cap, AND a second `EventAIUsageLimitReached` (without rejecting) when the post-call check shows the user just crossed the cap on this request (the current request still returns 200 since the upstream call already succeeded; the NEXT request will hit the pre-flight 429).
  - **`ListConversations`** — `GET` filter `{tenantId, userId}`, sort `updatedAt: -1`, paginated `limit`/`offset`, returns `{conversations, total, limit, offset}`.
  - **`GetConversation`** — `GET` filter `{tenantId, _id}` for the conversation + `{tenantId, conversationId}` for messages sorted `createdAt: 1`. Returns `{conversation, messages}`.
  - **`DeleteConversation`** — `DELETE` filter `{tenantId, _id}`. Deletes messages first (DeleteMany on `{tenantId, conversationId}`) then the conversation (DeleteOne on `{tenantId, _id}`) so a partial failure leaves a recoverable conversation rather than orphaned messages. Returns `{success, conversationId, deletedMessages}`.
  - **`GetUsage`** — `GET` filter `{tenantId, userId, date: {$gte, $lte}}` with `?from=&to=` YYYY-MM-DD query params (defaults to last 30 days). Returns `{usage: AIUsageStats[], totalRequests, totalTokens, dailyLimit, remainingTokens, from, to}`.
  - **`GenerateCourseOutline`** — `POST` body `{topic, level?, lessonsCount?}`. Pre-flight quota check, builds a strict-JSON system prompt + a topic/level/lessonsCount user prompt, calls OpenAI with `max_tokens=2048`, `temperature=0.7`, strips markdown fences via `extractJSON`, decodes into `{title, description, lessons: [{title, summary}]}`, meters the request, returns `{outline}`. On JSON parse failure returns 502 with the parse error.
  - **`GenerateQuiz`** — `POST` body `{lessonId?, topic, questionCount?, questionTypes?}`. When `lessonId` is supplied, fetches the Lesson (title + truncated content) via `db.Lessons().FindOne({tenantId, _id})` for grounding context — silent fallthrough if the lesson isn't found so the quiz still generates against the topic alone. Pre-flight quota check, builds a strict-JSON system prompt + topic/types/lesson-context user prompt, calls OpenAI, parses into `{questions: [{type, question, options, correctAnswer, explanation}]}`, meters, returns `{quiz}`.
- Verified build with `go build ./...` from `/home/z/my-project/repos/lastsaas/backend` — exit code 0, zero output.
- Verified `go vet ./internal/api/handlers/` — only the pre-existing `tenant_test.go:61` / `tenant_test.go:71` warnings ("using resp1/resp2 before checking for errors") which are unrelated to Phase 6 (also noted in the P6-A1 worklog as pre-existing). Zero new diagnostics from `ai_handler.go`.
- Iterated once on `truncateForPrompt`: the initial implementation had a buggy loop condition (`!strings.HasPrefix(s[cut:], "")` is always false because `HasPrefix(_, "")` is always true). Replaced with the correct continuation-byte walk-back (`for cut > 0 && s[cut]&0xC0 == 0x80 { cut-- }`) — this finds the nearest preceding UTF-8 leading byte and produces a rune-boundary cut. Re-verified the build still passes.

Stage Summary:
- Created `ai_handler.go` with 7 HTTP handlers (`Chat`, `ListConversations`, `GetConversation`, `DeleteConversation`, `GetUsage`, `GenerateCourseOutline`, `GenerateQuiz`) + 8 internal helpers (`requireContext`, `getOpenAIKey`, `getOpenAIModel`, `getOpenAIEndpoint`, `requireOpenAIKey`, `callOpenAI`, `buildSystemPrompt`, `truncateTitle`, `normalizeToDate`, `getTodayUsage`, `incrementDailyUsage`, `getConversationHistory`, `extractJSON`, `truncateForPrompt`) + 6 wire/DTO types (`openAIMessage`, `openAIRequest`, `openAIUsage`, `openAIResponse`, `openAIErrorBody`, and the public DTOs `chatRequest`/`chatResponse`/`chatUsage`/`courseOutlineRequest`/`courseOutline`/`courseOutlineLesson`/`courseOutlineResponse`/`quizRequest`/`quizQuestion`/`quizPayload`/`quizResponse`).
- Build status: PASS (`go build ./...` exits 0, zero output).
- Vet status: PASS on `ai_handler.go` — only pre-existing `tenant_test.go` warnings remain (unrelated, also documented in P6-A1 worklog).
- Routes: `POST /api/lms/ai/chat`, `GET /api/lms/ai/conversations`, `GET /api/lms/ai/conversations/{id}`, `DELETE /api/lms/ai/conversations/{id}`, `GET /api/lms/ai/usage`, `POST /api/lms/ai/generate-course-outline`, `POST /api/lms/ai/generate-quiz`. (Route wiring in `cmd/server/main.go` is left to a separate task — the P6-A4 contract is handler creation only.)
- OpenAI integration: used `net/http` directly (NOT `github.com/sashabaranov/go-openai`) because the package is not in `go.mod`. No new dependencies added; no `go mod tidy` required.
- OpenAI key handling: if `OPENAI_API_KEY` is unset, `Chat`/`GenerateCourseOutline`/`GenerateQuiz` return HTTP 503 with `"AI service not configured. Set OPENAI_API_KEY environment variable."` — no crash, no upstream call attempted.
- Daily quota: `aiDailyTokenLimit = 100000` tokens per user per day (UTC), enforced via pre-flight check on `Chat` + both generators. When the cap is hit pre-flight: emit `EventAIUsageLimitReached` + return 429. When the cap is crossed mid-request (post-call): emit `EventAIUsageLimitReached` but still return 200 for this request (the upstream call already succeeded); the next request will hit the pre-flight 429.
- All tenant-scoped queries filter by `tenantId`. `Chat`/`ListConversations`/`GetUsage` additionally filter by `userId` so users only see their own conversations and usage. `GetConversation`/`DeleteConversation` filter by `{tenantId, _id}` per the spec — the conversation is additionally verified to exist before any delete in `DeleteConversation`. `GenerateCourseOutline`/`GenerateQuiz` filter their optional `Lesson` fetch by `{tenantId, _id}` so cross-tenant lesson grounding is impossible.

Notes for downstream agents:
- The route wiring (`lmsAPI.HandleFunc("/ai/chat", aiHandler.Chat).Methods("POST")`, etc.) is NOT done in this task — `cmd/server/main.go` does not yet instantiate `NewAIHandler(database, emitter)` or register any `/api/lms/ai/*` routes. A separate route-wiring task should add a `aiHandler := handlers.NewAIHandler(database, emitter)` line near the existing `proEngagementGamificationHandler` declaration (line ~459 of `main.go`) and register the 7 routes. The handler method signatures match the spec verbatim so wiring is mechanical.
- The daily token limit (`aiDailyTokenLimit = 100000`) is a per-user constant; if per-tenant metering is needed later, swap `getTodayUsage(ctx, ctx.TenantID, ctx.UserID)` for an aggregation over `AIUsageStats` filtered by `{tenantId, date}` (dropping the `userId` filter). The `AIUsageStats` unique index is `(tenantId, userId, date)` so per-user queries are index-covered; per-tenant aggregation would require a new non-unique index on `(tenantId, date)` for efficiency.
- The OpenAI passthrough uses the standard Chat Completions API (`/v1/chat/completions`) with the `gpt-4` model by default. To switch to a cheaper model for cost optimization, set `OPENAI_MODEL=gpt-3.5-turbo` (or any other model id). The `temperature` and `max_tokens` are hardcoded per endpoint (0.7/1024 for Chat, 0.7/2048 for generators) — make these env-configurable if A/B testing is required.
- The generators (`GenerateCourseOutline`, `GenerateQuiz`) do NOT persist a conversation or messages — they are stateless JSON generators that meter against `AIUsageStats` only. If audit trail of generated outlines/quizzes is required, wrap the response in a separate `AIGenerationRecord` collection (not in scope of P6-A1 models).
- The `extractJSON` helper strips ```` ```json ```` / ```` ``` ```` fences and trailing ```` ``` ````, which is the most common OpenAI JSON-wrapping behaviour. If a model returns the JSON inside a `<json>...</json>` tag or with leading prose, the `json.Unmarshal` will fail with a parse error and the handler returns 502 — the frontend should then fall back to displaying the raw model output or retrying with a stricter prompt.
- `GetConversation` and `DeleteConversation` filter by `{tenantId, _id}` per the spec, NOT `{tenantId, _id, userId}`. This means any authenticated user in the tenant can read/delete any other user's conversation in the same tenant. If finer-grained per-user isolation is required, add `"userId": ctx.UserID` to both filters — the `AIConversation.UserID` field is already populated on insert so the filter would just work.

---
Task ID: P6-A6
Agent: Frontend Reports Dashboard
Task: Create reports-dashboard with 6 tabs + charts + CSV export

Work Log:
- Read `/home/z/my-project/worklog.md` P6-A2 section to confirm the established Phase 6 reports hook contract: query hooks return `{ data, loading, error, refetch }`, mutation hooks return `{ data, loading, error, mutate, reset }`, all parameters passed at call-time so a single hook instance can act on any row. Confirmed the 6 report query hooks (`useOverviewReport`, `useSalesReport`, `useEnrollmentReport`, `useCompletionReport`, `useCourseReport`, `useStudentReport`) all accept an optional `ReportFilters` and refetch on `argsKey([params])` change; the 3 reports mutations are `useExportReport({ reportType, params? }) → { downloadUrl }`, `useSaveReport({ name, reportType, config }) → SavedReport`, `useDeleteSavedReport(id) → { success }`; `useSavedReports()` returns `SavedReport[]`.
- Read the Phase 6 type block at `src/types/lms.ts` lines 1846-2000 to capture the exact field names of every report payload (`OverviewReport.dailySeries.{date,revenueCents,enrollments,orders}`, `SalesReport.{totalSalesCents,totalOrders,avgOrderValueCents,refundRate,topCourses,paymentMethods,dailySeries}`, `EnrollmentReport.{totalEnrollments,activeEnrollments,completedEnrollments,cancelledEnrollments,enrollmentGrowth,completionRate,topCourses,dailySeries.{enrollments,completions}}`, `CompletionReport.{overallCompletionRate,avgCompletionDays,courses.{enrolledCount,completedCount,completionRate,avgScore,dropoffRate},funnel.{stage,count,percentage}}`, `CourseReport.{courses.{courseId,title,instructorName,enrollments,revenueCents,avgRating,completionRate,status},totalCourses,publishedCourses,draftCourses}`, `StudentReport.{students.{studentId,name,email,enrollments,completedCourses,totalSpentCents,lastActiveAt},totalStudents,activeStudents,newStudentsThisMonth}`, `ReportFilters.{from,to,courseId,instructorId,category}`, `ReportType` = "overview"|"sales"|"enrollments"|"completion"|"courses"|"students"|"instructors", `SavedReport.{id,tenantId,name,reportType,config,scheduleCron?,lastRunAt?,createdBy,createdAt,updatedAt}`).
- Read `src/app/pages/apps/instructor-dashboard/index.tsx` (339 lines) as the canonical 2-column sidebar layout pattern: `Page` wrapper + top header (size-9 primary icon + title/subtitle + trailing badge) + `<aside className="w-60">` sidebar with `ScrollShadow`-wrapped `<nav>` of `Button variant="flat" color="primary"|"neutral"` items + `<main>` with breadcrumb strip + content `ScrollShadow`. Verified the tailux `Button` does NOT accept a `size` prop (uses padding classes instead) — adjusted the `DateRangePicker` preset chips to use explicit `h-7 px-2.5 text-xs` classes.
- Read `src/app/pages/apps/payouts-admin/index.tsx` (776 lines) as the canonical admin-dashboard pattern with `EmptyState`/`ErrorState`/`LoadingState` usage, `Card skin="bordered"` for tables, `Table hoverable` + `THead`/`TBody`/`Tr`/`Th`/`Td` rows, and `Badge color="primary"|"success"|"warning"|"error"|"neutral" variant="soft"` chips.
- Read `src/components/ecommerce/RevenueChart.tsx` (199 lines) as the canonical div-based bar-chart pattern: `flex h-48 items-end justify-between gap-0.5` row of `min-w-[6px] flex-1` columns with proportional `style={{ height: '${pct}%' }}` bars + `group-hover:block` tooltip — replicated this approach in `SimpleBarChart.tsx` so no chart library is pulled into the bundle.
- Created `DateRangePicker.tsx` (177 lines) — two native `<input type="date">` controls + 7 preset chips (Last 7/30/90 days, This Month, Last Month, This Year, All Time). Presets compute `[from, to]` ISO date strings via a small `toIso()` helper; the active preset is auto-detected by comparing the current `from`/`to` against each preset's `compute()`. Props: `{ from?, to?, onChange, className? }`. No date library (keeps the bundle small).
- Created `KpiCard.tsx` (114 lines) — reusable KPI card with `{ label, value, growth?, icon?, color?, subtitle?, className? }`. Renders the label (uppercase xs), the value (2xl font-semibold), an optional icon well in the top-right (sized 10×10, tinted by color), and an optional growth chip (green up-arrow / red down-arrow with `Math.abs(growth).toFixed(1)%`). Color variants cover all 7 `ColorType` values; uses `Card` so it inherits the active theme skin.
- Created `SimpleBarChart.tsx` (112 lines) — div-based vertical bar chart with `{ data: Array<{ label, value, formattedValue?, barClassName? }>, color?, height?, className? }`. Each bar's height is `Math.max(value/peak * 100, 1.5)%`; bars use `group-hover:block` tooltips showing the label + formatted value. Falls back to a dashed empty-state when `data.length === 0`.
- Created `SimpleLineChart.tsx` (170 lines) — inline-SVG line chart with `{ data, color?, height?, className? }`. Renders a `polyline` + per-point `circle` markers + a faint filled `path` beneath the line, all positioned in a 0..100 viewBox with `preserveAspectRatio="none"` so the SVG scales fluidly. Three reference grid lines + hover tooltips via a `useState(hoverIdx)` and an invisible fat hit-target circle (r=4) on each data point. Y-axis peak/min annotation rendered beneath.
- Created `ExportButton.tsx` (88 lines) — CSV export trigger wrapping `useExportReport().mutate({ reportType, params })`. On success, navigates the browser to the returned `downloadUrl` via `window.location.href = result.downloadUrl` (the pre-signed S3 URL downloads without leaving the dashboard). Shows a loading spinner + "Preparing…" label while the export is being prepared and surfaces any error inline beneath the button.
- Created `OverviewTab.tsx` (220 lines) — 6 KPI cards (Total Revenue, Net Revenue, Total Orders, Total Enrollments, Completion Rate, Avg Rating) in a responsive grid + a `SimpleBarChart` of the last 30 days of `dailySeries.revenueCents` + a recent-activity feed derived from the latest 8 daily-series entries with non-zero orders/enrollments. Each KPI surfaces the matching growth indicator (`revenueGrowth`/`enrollmentGrowth`) when available.
- Created `SalesTab.tsx` (220 lines) — 4 KPI cards (Total Sales, AOV, Refund Rate, Total Orders) + a `SimpleLineChart` of `dailySeries.salesCents` + a Top Courses table (title, sales, orders, % of total) + a Payment Methods panel with per-gateway progress bars (`bg-primary-500`/`bg-success-500`/… cycled through 6 colors) and order counts + total amount.
- Created `EnrollmentsTab.tsx` (250 lines) — 4 KPI cards (Total/Active/Completed/Completion Rate) + two `SimpleLineChart`s side-by-side (daily new enrollments in primary-500, daily completions in success-500) + a Top Courses by Enrollment table (title, enrolled, completed, rate) + a Status breakdown panel with three progress bars (Active/Completed/Cancelled) and per-status icons.
- Created `CompletionTab.tsx` (290 lines) — top strip with an inline-SVG `ProgressRing` showing `overallCompletionRate` as a big circular gauge + a KpiCard for `avgCompletionDays` + a KpiCard for courses-tracked; a Funnel panel with horizontal bars whose widths are proportional to each stage's count (peak stage = full width); and a per-course completion table (course, enrolled, completed, completion %, avg score, dropoff %). Used `AdjustmentsHorizontalIcon` (NOT `FunnelChartIcon` — verified the latter is NOT exported from `@heroicons/react/24/outline` via `node -e "const h = require('@heroicons/react/24/outline'); console.log(typeof h.FunnelChartIcon)"` → `undefined`).
- Created `CoursesTab.tsx` (290 lines) — 3 summary cards (Total/Published/Draft) + a sortable + searchable courses table (title, instructor, enrollments, revenue, rating, completion rate, status). Sorting is client-side via `useState<SortKey,SortDir>`; the `SortTh` helper renders the active column with an up/down arrow. Status column uses a `Badge color="success"|"warning"|"neutral" variant="soft" capitalize` chip.
- Created `StudentsTab.tsx` (290 lines) — 3 summary cards (Total/Active/New This Month) + a sortable + searchable students table (name, email, enrollments, completed, total spent, last active). Reuses the same `SortTh` pattern; `lastActiveAt` is rendered as a relative time ("just now", "5m ago", "3h ago", "2d ago", or a localized date for >30d).
- Created `SavedReportsTab.tsx` (190 lines) — `useSavedReports()` + `useDeleteSavedReport()` table (name, type badge, last run relative time, created date, actions). "Run" calls `onRun(report)` which the parent uses to switch tabs + apply the saved filter config; "Delete" calls `mutate(id)` and refetches. Type badge color is mapped per `ReportType`.
- Created `index.tsx` (355 lines) — main dashboard shell. Sidebar with 7 nav items (Overview / Sales / Enrollments / Completion / Courses / Students / Saved Reports) using the `instructor-dashboard`/`payouts-admin` button pattern. Top toolbar holds the `DateRangePicker`, a free-text Course filter, `ExportButton` (per-tab `reportType`), and a "Save Report" button that expands an inline save dialog (`<input>` for name + Save/Cancel buttons + `useSaveReport().mutate({ name, reportType: activeItem.reportType, config: { filters, tab: active } })`). Saved-report "Run" callbacks switch to the matching tab and apply `config.filters`. Active tab body is switched via a `{active === "X" && <XTab filters={filters} />}` ladder.
- Registered the route in `src/app/router/protected.tsx` immediately after the `assignment-grading` entry (line 1331): `{ path: "reports-dashboard", lazy: async () => ({ Component: (await import("@/app/pages/apps/reports-dashboard")).default }) }` — matches the established pattern for `payouts-admin`, `orders-admin`, etc. so the page is reachable at `/apps/reports-dashboard` with code-splitting.
- Ran `npx tsc --noEmit` from `/home/z/my-project/repos/tailux/tailux-main` — exit code 0, zero diagnostics. No type errors in any of the 13 new files; no type errors introduced in `protected.tsx` either.

Stage Summary:
- Created 13 files in `src/app/pages/apps/reports-dashboard/`: `index.tsx`, `OverviewTab.tsx`, `SalesTab.tsx`, `EnrollmentsTab.tsx`, `CompletionTab.tsx`, `CoursesTab.tsx`, `StudentsTab.tsx`, `SavedReportsTab.tsx`, `DateRangePicker.tsx`, `KpiCard.tsx`, `SimpleBarChart.tsx`, `SimpleLineChart.tsx`, `ExportButton.tsx`. Plus 1 route registration in `src/app/router/protected.tsx`.
- TypeScript check: PASS (`npx tsc --noEmit` exits 0, zero diagnostics).
- Routes: `/apps/reports-dashboard` (registered in `protected.tsx` line ~1331).
- Charts: 100% div-based or inline-SVG — NO chart library added to the bundle (kept `recharts`/`chart.js`/etc. out of the dependency tree).
- Hooks used: `useOverviewReport`, `useSalesReport`, `useEnrollmentReport`, `useCompletionReport`, `useCourseReport`, `useStudentReport`, `useExportReport`, `useSaveReport`, `useSavedReports`, `useDeleteSavedReport` (10/10 reports hooks from `useReportsAI.ts`).
- All tabs: loading (`LoadingState`) + error (`ErrorState` with `onRetry={refetch}`) + empty (`EmptyState`) states wired. All components use `dark:` variants throughout.

---
Task ID: P6-A7
Agent: Frontend AI Assistant + Migration Wizard
Task: Create tutor-ai + migration-wizard page areas

Work Log:
- Read `/home/z/my-project/worklog.md` P6-A2 section to confirm the Phase 6 hooks contract: `useAIConversations()`, `useAIConversation(id?)` (returns `{ conversation, messages }`), `useSendAIMessage()` (vars `AISendMessageInput` → `AISendMessageResult` with `conversationId`, `message`, `usage { tokensUsed, remainingTokens }`), `useDeleteAIConversation()` (vars `id`), `useAIUsage(params?)`, `useGenerateCourseOutline()`, `useGenerateQuiz()`, `useMigrations(params?)`, `useCreateMigration()`, `useStartMigration()`, `useCancelMigration()`, `useMigrationLogs(id?, params?)`. All hooks follow `{ data, loading, error, refetch | mutate, reset }` shape with no React Query.
- Read `src/app/pages/apps/ai-chat/` (index.tsx + Conversation/Message.tsx) as the established AI chat template: `<Page title="…"><Sidebar /><main className="main-content transition-content flex h-screen min-w-0 flex-col supports-[height:1dvh]:h-dvh">…</main></Page>`, right-aligned user bubble + left-aligned assistant bubble with `RiRobot2Line` bot avatar, `Avatar size={10} initialColor="info"` pattern.
- Read `src/app/pages/apps/instructor-dashboard/index.tsx` as the canonical 2-column sidebar + content layout (sticky header with primary-tinted icon, `ScrollShadow` content area, sidebar with `Button` nav items + `clsx` active state).
- Read `src/app/pages/apps/prerequisite-manager/AddPrerequisiteModal.tsx` for the established `@headlessui/react` modal pattern: `Transition appear show as={Fragment}` + `Dialog as="div" className="relative z-[100]"` + `TransitionChild` for backdrop + `DialogPanel` for the card; `Button isIcon variant="flat" color="neutral"` for the close button.
- Read `src/components/ui/index.ts` + `Form/index.ts` to confirm available primitives: `Button`, `Card`, `Badge`, `Input`, `Select`, `Textarea`, `Checkbox`, `Switch`, `Avatar`, `Spinner`, `Progress`, `ScrollShadow`, `Table`. Read `Card/index.tsx`, `Button/index.tsx`, `Badge/index.tsx`, `Progress/index.tsx`, `Input.tsx`, `Select.tsx`, `Textarea.tsx`, `Checkbox.tsx`, `Avatar.tsx`, `ScrollShadow/index.tsx`, `Spinner/Spinner.tsx` for the exact prop surfaces (e.g. `Button color="primary" variant="soft" isIcon`, `Card skin="none"`, `Progress value={n} color="primary"`, `ScrollShadow` forwards a ref to the underlying div).
- Read `src/components/lms/index.ts` for the shared `EmptyState`, `LoadingState`, `ErrorState` helpers — used in every loading/empty/error branch in both new pages.
- Read `src/types/lms.ts` Phase 6 block (lines 2002-2117) to confirm the exact shape of `AIConversation`, `AIMessage` (`role: "user" | "assistant" | "system"`), `AISendMessageInput` (`conversationId?`, `message`, `context? { courseId?, lessonId?, action? }`), `AISendMessageResult` (`conversationId`, `message`, `usage { tokensUsed, remainingTokens }`), `AIUsageStats` (`date`, `totalRequests`, `totalTokens`, `estimatedCostCents`), `MigrationPlatform` (6 values), `MigrationJobStatus` (5 values), `MigrationJob` (with `totalCourses`/`migratedCourses`/etc. counters + `sourceConfig`), `MigrationLog` (`level: "info" | "warning" | "error"` + `entity`/`sourceId`/`targetId`), `CreateMigrationInput` (`platform` + `sourceConfig { dbHost?, dbName?, dbUser?, dbPassword?, apiKey?, apiUrl?, filePath? }`).
- Created 7 files in `src/app/pages/apps/tutor-ai/`:
  - `ChatMessage.tsx` — single-message bubble (right-aligned primary bubble for user, left-bordered neutral bubble for assistant with `RiRobot2Line` avatar, centred pill for system messages, timestamp + token count footer).
  - `ChatInput.tsx` — auto-resizing textarea (max 200px), Enter-to-send / Shift+Enter-newline, 4 quick-action chips (Generate Course Outline / Generate Quiz / Improve Content / Summarize) that either delegate to `onQuickAction` or pre-fill the textarea, send button disabled on empty/`disabled`.
  - `ConversationList.tsx` — left sidebar with "New Chat" CTA + conversation list (relative timestamp + hover-revealed delete button + active-state highlight + loading/empty/error states via `@/components/lms`).
  - `UsagePanel.tsx` — right sidebar with `useAIUsage({ from, to })` for the last 7 days, today's-quota progress bar, window-total tokens + estimated cost cards, mini bar chart of daily token usage; accepts an optional `remainingTokens` prop surfaced from the most-recent `useSendAIMessage` reply.
  - `CourseOutlineGenerator.tsx` — `@headlessui/react` modal with topic / level / lessonsCount form → `useGenerateCourseOutline().mutate(...)` → renders the result as a module/lesson tree (with JSON fallback for unknown shapes) → "Create Course" CTA navigates to `/apps/course-builder` with `location.state.aiOutline`.
  - `QuizGenerator.tsx` — modal with topic / questionCount / multi-select `questionTypes` form → `useGenerateQuiz().mutate(...)` → renders the result as a question list with options/answer/explanation → "Create Quiz" CTA navigates to `/apps/quiz-builder` with `location.state.aiQuiz`.
  - `index.tsx` — 3-column layout (left `ConversationList`, centre chat with optimistic message append + typing-dots placeholder, right `UsagePanel` + Context panel). State machine: new-chat mode (`selectedId === null`) sends the first message via `useSendAIMessage` and on success switches to active mode by setting `selectedId` from `result.conversationId`; active mode renders `useAIConversation(selectedId).data.messages` with optimistic overlay merged via id-set diff. Both generator modals are opened from the top-bar buttons or from the ChatInput quick-action chips.
- Created 5 files in `src/app/pages/apps/migration-wizard/`:
  - `PlatformSelector.tsx` — 6-card grid (LearnDash / LifterLMS / LearnPress / WooCommerce / Tutor LMS / CSV Import) with heroicon + label + description + kind badge (MySQL / API / CSV), active card highlighted with primary tint. Exports `getPlatformLabel` + `getPlatformKind` helpers used by the wizard review step and the source-config form.
  - `SourceConfigForm.tsx` — dynamic form keyed off `getPlatformKind(platform)`: MySQL platforms get host/port/dbName/user/password (+ WordPress URL for woocommerce), API platforms get apiUrl/apiKey, CSV gets a drag-drop file input + remote URL field. Exports `SourceConfig` type (extends `CreateMigrationInput["sourceConfig"]` with `dbPort` + `wpUrl` form-only fields), `validateSourceConfig` (returns `null` when valid or a friendly error string), and `toCreateSourceConfig` (strips the form-only fields back into the wire shape).
  - `MigrationJobCard.tsx` — presentational card: platform icon + label + Job-#id, status badge with coloured dot (`pending`/`running`/`completed`/`failed`/`cancelled`), `Progress` bar (`migratedCourses / totalCourses`), 4-cell count grid (courses / lessons / quizzes / students), error block on `failed`, footer with startedAt timestamp + View Logs / Cancel (running/pending) / Retry (failed/cancelled) actions.
  - `MigrationLogs.tsx` — `@headlessui/react` modal with `useMigrationLogs(jobId)`, level-filter chips (All / Info / Warning / Error), colour-coded log rows (info=gray, warning=warning, error=error) with timestamp + entity/sourceId/targetId meta, auto-scroll-to-bottom via `useRef` + `useEffect` on filtered length.
  - `index.tsx` — 2-column layout (sidebar New Migration / Migration History tabs + content panel). New Migration is a 3-step wizard with a `Stepper` (numbered + checkmark for completed steps): step 1 `PlatformSelector`, step 2 `SourceConfigForm` (gated by `validateSourceConfig`), step 3 review summary card + Start Migration button that calls `useCreateMutation().mutate()` then `useStartMigration().mutate(id)` and jumps to the History tab on success. History shows summary pills (Total / Pending / Running / Completed / Failed / Cancelled) + a `MigrationJobCard` grid with Cancel / Retry / View Logs actions wired to the corresponding hooks.
- Registered two routes in `src/app/router/protected.tsx` (after `reports-dashboard`, before the closing `]` of the apps route group): `tutor-ai` → `@/app/pages/apps/tutor-ai`, `migration-wizard` → `@/app/pages/apps/migration-wizard`. Both use the same `lazy: async () => ({ Component: (await import(...)).default })` pattern as the existing Phase 4/5/6 routes.
- Ran `npx tsc --noEmit` from `/home/z/my-project/repos/tailux/tailux-main` — exit code 0, zero diagnostics. No new errors introduced by the 12 new files or the route additions.

Stage Summary:
- Created 12 files across 2 page areas (7 in `src/app/pages/apps/tutor-ai/`, 5 in `src/app/pages/apps/migration-wizard/`).
- TypeScript check: PASS (`npx tsc --noEmit` exits 0, zero diagnostics).
- Routes: `/apps/tutor-ai`, `/apps/migration-wizard` (both lazy-loaded via the apps route group in `protected.tsx`).

---
Task ID: P6-A3
Agent: Backend Reports Handlers
Task: Create reports_handler.go with 6 report types + CSV export + saved reports CRUD

Work Log:
- Read `/home/z/my-project/worklog.md` end-to-end for P5-A3 (handler pattern) + P6-A1 (models + collections + events) + P6-A2 (frontend contract). Confirmed the per-resource handler struct + constructor + `requireCtx` helper + `parsePositiveInt` + `mux.Vars(r)["id"]` + `bson.M{"_id": ..., "tenantId": ...}` filter + `events.Event{Type, Timestamp, Data}` emission pattern.
- Read `internal/api/handlers/ecommerce_revenue.go` end-to-end as the canonical revenue/report pattern (EcommerceRevenueHandler with `db` + `emitter` fields, `requireCtx` wrapping `getLMSContext`, `parseTimeQuery` free helper, daily-series bucketing via `map[string]*dayBucket` + `sort.Slice`).
- Read `internal/api/handlers/lms.go` lines 1-130 to confirm `getLMSContext` returns `lmsContext{TenantID, UserID, IsInstructor}` (with dev-fallback to `000000000000000000000001` / `000000000000000000000002` when `LASTSAAS_ENV=dev`) and that `parsePositiveInt` lives at lines 4000-4015.
- Read `internal/models/reportsai.go` to confirm `ReportType` enum (`overview|sales|enrollments|completion|courses|students|instructors`), `ReportSnapshot` (immutable point-in-time capture) and `SavedReport` (re-runnable config + optional `ScheduleCron`) struct shapes.
- Read `internal/models/lms.go` (Course/Enrollment/LessonProgress/QuizAttempt/CourseReview/Order/OrderItem/OrderStatus/OrderItemType) and `internal/models/ecommerce.go` (PaymentTransaction/Refund/RevenueLedgerEntry/WithdrawalRequest) to lock down the field names used in aggregations.
- Read `internal/models/membership.go` to confirm `RoleUser` is the student role (no `RoleStudent` constant) — used for the Students report `tenantMemberships.role=user` filter.
- Read `internal/events/reportsai_events.go` to confirm `EventReportGenerated`/`EventReportSaved`/`EventReportScheduled` constants; the first two are emitted by this handler (EventReportScheduled is reserved for the cron scheduler that P6-A4 will add).
- Created `/home/z/my-project/repos/lastsaas/backend/internal/api/handlers/reports_handler.go` (~1665 lines after gofmt) with:
  * `ReportsHandler` struct + `NewReportsHandler(database, emitter)` constructor + `requireCtx` helper (mirrors EcommerceRevenueHandler pattern).
  * Shared `reportParams` struct (From/To/CourseID/InstructorID/Period/Limit/Offset) + `parseReportParamsFromQuery` (URL ?from/?to/?courseId/?instructorId/?limit/?offset, defaults to last 30 days, `to` extended to end-of-day) + `parseReportParamsFromMap` (POST /export body shape) + `finalizeReportParams` shared normaliser.
  * `reportResult` struct bundling `Response` (JSON payload) + `Rows` + `Columns` (CSV export input) so each `compute*` method services both the HTTP handler and the CSV exporter with a single pass over the DB.
  * `emitReportGenerated` helper that fires `EventReportGenerated` with a scalar-only summary (large arrays stripped to keep the event small).
  * `dailySeriesBuilder` helper (UTC day-keyed `map[string]map[string]interface{}` with a `zero()` factory + `add(t, mutate)` + `build()` returning a date-sorted slice) used by Overview/Sales/Enrollments reports.
  * `growthPercent(current, previous)` divide-by-zero-safe growth helper.
  * `findAll[T any]` generic helper (Go 1.18+ type parameter) wrapping `coll.Find + cursor.All` to remove the cursor/decode boilerplate that would otherwise repeat in every `compute*` method.
  * 6 `compute*` methods (Overview/Sales/Enrollments/Completion/Courses/Students), each: builds the tenantId-scoped filter, applies optional courseId/instructorId filters, fetches the relevant collections in parallel via `findAll`, aggregates in Go, builds the daily series (where applicable), computes previous-period growth (where applicable), and packages the result into `reportResult`.
  * 6 HTTP handlers (`OverviewReport`/`SalesReport`/`EnrollmentsReport`/`CompletionReport`/`CoursesReport`/`StudentsReport`) that call the `compute*` method, emit `EventReportGenerated`, and respond with the result.
  * `ExportReport` (POST /export) that decodes `{reportType, params}` from the body, dispatches to the matching `compute*` method, encodes the result rows as CSV via `buildCSV` (using `encoding/csv` + `sanitizeCSVField` from helpers.go for formula-injection defence), base64-encodes the CSV, and returns `{downloadUrl, reportType, rows}` with a `data:text/csv;charset=utf-8;base64,...` URL the browser can navigate to directly.
  * `SaveReport` (POST /save) that validates name + reportType, inserts a `SavedReport` document with `primitive.NewObjectID()` + timestamps + `CreatedBy = ctx.UserID`, emits `EventReportSaved`, and responds 201 with the inserted doc.
  * `ListSavedReports` (GET /saved) that filters by `{tenantId}` (and optional `?reportType=`), paginates via `?limit=/?offset=` (default 50, max 200), sorts by `createdAt desc`.
  * `DeleteSavedReport` (DELETE /saved/{id}) that filters by `{_id, tenantId}` and returns 404 when `DeletedCount == 0`.
  * `isValidReportType` enum validator and the generic `findAll` helper at the bottom of the file.
- Modified `cmd/server/main.go`: (1) added `reportsHandler := handlers.NewReportsHandler(database, emitter)` instantiation after the Phase 5 handler block; (2) registered 10 routes under `lmsAPI` (`/admin/reports/{overview|sales|enrollments|completion|courses|students}` GET, `/admin/reports/export` POST, `/admin/reports/save` POST, `/admin/reports/saved` GET, `/admin/reports/saved/{id}` DELETE) immediately after the Phase 5 Legal consents block, with a comment noting the static-vs-wildcard ordering.
- Ran `gofmt -w` on both files (Write tool introduced spaces; gofmt enforces tabs) — verified `gofmt -l` is clean on both.
- Verified with `go build ./...` from `/home/z/my-project/repos/lastsaas/backend` — exit code 0, zero output.
- Verified with `go vet ./...` — zero new diagnostics from reports_handler.go or main.go (the only pre-existing vet warnings are in `tenant_test.go` lines 61/71 — unrelated to Phase 6).

Stage Summary:
- Created `internal/api/handlers/reports_handler.go` with 10 HTTP handler methods on `ReportsHandler` (6 report GETs + ExportReport POST + SaveReport POST + ListSavedReports GET + DeleteSavedReport DELETE) + 6 private `compute*` methods (one per report type) + 5 free helpers (`parseReportParamsFromQuery`, `parseReportParamsFromMap`, `finalizeReportParams`, `buildCSV`, `formatCSVValue`, `findAll[T]`) + 3 method helpers (`requireCtx`, `emitReportGenerated`, `dailySeriesBuilder{add,build}`) + 2 free helpers (`growthPercent`, `isValidReportType`).
- Modified `cmd/server/main.go`: 1 new handler instantiation (line 465) + 10 new route registrations (lines 736-745).
- Build status: PASS (`go build ./...` exits 0).
- Vet status: PASS (no new diagnostics from touched files; pre-existing `tenant_test.go` warnings at lines 61/71 are unrelated).
- gofmt status: PASS (`gofmt -l` clean on both files).
- Routes: GET /api/lms/admin/reports/{overview|sales|enrollments|completion|courses|students}, POST /api/lms/admin/reports/export, POST /api/lms/admin/reports/save, GET /api/lms/admin/reports/saved, DELETE /api/lms/admin/reports/saved/{id}.
- Events emitted: `EventReportGenerated` on every report run (6 GETs + 1 export); `EventReportSaved` on SaveReport. `EventReportScheduled` is reserved for the P6-A4 cron scheduler.
- CSV export: returns a `data:text/csv;charset=utf-8;base64,...` URL the browser can navigate to directly — no temp file or static-asset serving required. All string cells pass through `sanitizeCSVField` (from helpers.go) to defang CSV formula injection.

Notes for downstream agents:
- The 6 `compute*` methods are exported as private (lowercase) but take `(ctx context.Context, lctx lmsContext, p reportParams)` so a future scheduler (P6-A4) can call them directly from a goroutine to materialise a `ReportSnapshot` on the cron cadence — no HTTP request needed. The `reportResult.Response` map fits straight into `ReportSnapshot.Summary`; `reportResult.Rows` fits into `ReportSnapshot.Data`.
- The ExportReport endpoint returns a `data:` URL rather than a pre-signed S3 URL because the backend has no S3 integration yet. If/when S3 is added, swap the `data:` URL for a pre-signed PUT URL — the response shape (`{downloadUrl, reportType, rows}`) stays the same.
- The previous-period growth computation uses `p.To.Sub(p.From)` as the duration and shifts the window back by that amount. For a 30-day window from 2026-07-01 to 2026-07-31, the previous period is 2026-06-01 to 2026-06-30. This matches the RevenueReport (Phase 3) convention.
- `findAll[T any]` is a generic helper at the bottom of the file. If any other handler in the package wants to reuse it, it's already in the `handlers` package — just call `findAll[models.Foo](ctx, h.db.Foos(), filter)`.
- The `students` report loads ALL tenant enrollments + paid orders + lesson-progress rows in memory to aggregate per-student. For tenants with > 10k students this could be slow; the `?limit=` cap (default 100, max 500) bounds the student count but not the enrollment/order/progress scan. If a tenant hits performance issues, swap the in-memory aggregation for MongoDB `$group` aggregation pipelines.
- The `courses` report revenue aggregation uses `items.referenceId: {$in: courseIDs}` (dotted path into the items array) rather than `$elemMatch` — this matches any order with at least one matching item, which is what we want.

---
Task ID: P6-A5
Agent: Backend Migration Handlers
Task: Create migration_handler.go with job CRUD + platform migrations

Work Log:
- Read `/home/z/my-project/worklog.md` P6-A1 entry to confirm the Phase 6 model/collection/event surface: `MigrationJob` / `MigrationLog` / `MigrationMapping` structs and the `MigrationPlatform` / `MigrationJobStatus` enums in `internal/models/reportsai.go`; `db.MigrationJobs()` / `db.MigrationLogs()` / `db.MigrationMappings()` accessors in `internal/db/reportsai_collections.go`; `EventMigrationJobStarted` / `EventMigrationJobCompleted` / `EventMigrationJobFailed` / `EventMigrationBatchCompleted` constants in `internal/events/reportsai_events.go` (and confirmed the 4 legacy `migration.*` constants in `lms_events.go` are intentionally NOT re-declared).
- Read `internal/api/handlers/proengagement_gamification.go` lines 1-330 as the canonical handler pattern: `type XHandler struct { db *db.MongoDB; emitter events.Emitter }`, `NewXHandler(database *db.MongoDB, emitter events.Emitter) *XHandler` constructor, `requireContext(w, r) (lmsContext, bool)` helper that wraps `getLMSContext(r)` and rejects zero `UserID`, `respondWithJSON`/`respondWithError` from `helpers.go`, `parsePositiveInt(r, key, default, max)` from `lms.go` line 4002 for pagination, `mux.Vars(r)["id"]` for path params, `bson.M{"tenantId": ctx.TenantID}` filter on every query, `options.Find().SetLimit().SetSkip().SetSort()` for paginated reads, `h.emitter.Emit(events.Event{Type, Timestamp, Data})` for notifications.
- Read `internal/api/handlers/lms.go` lines 56-130 to confirm the `getLMSContext(r)` helper: preferred path is `middleware.GetTenantFromContext` + `middleware.GetUserFromContext` + `middleware.GetMembershipFromContext`; fallback to `mux.Vars(r)["tenantId"]`; dev fallback pins to `000000000000000000000001` tenant + `000000000000000000000002` user. `lmsContext` struct carries `{TenantID, UserID, IsInstructor}`.
- Read `internal/models/lms.go` lines 41-336 to confirm the target entity shapes for migration: `Course` (TenantID/InstructorID/Title/Slug/Description/Excerpt/Status/PriceType/Currency/CreatedAt/UpdatedAt — Slug must be unique within tenant), `Lesson` (CourseID/TopicID/InstructorID/Title/Slug/Content/LessonType), `Quiz` (CourseID/TopicID/LessonID/InstructorID/Title/Description/Settings), `Question`, `Enrollment` (CourseID/StudentID/Status). Read lines 873-908 to confirm the legacy `Migration` struct + `MigrationStatus` enum remain in place for back-compat — the new `MigrationHandler` does NOT touch `lms_migrations` (legacy collection); it uses `lms_migration_jobs` / `lms_migration_logs` / `lms_migration_mappings` exclusively.
- Verified `github.com/go-sql-driver/mysql` is NOT in `go.mod` / `go.sum` and the module cache at `/home/z/go/pkg/mod/github.com/go-sql-driver/` does not exist. Per the task spec fallback, designed the MySQL importers (LearnDash / LifterLMS / LearnPress / WooCommerce) against stdlib `database/sql` with the driver name `"mysql"`; at runtime `sql.Open("mysql", dsn)` returns an "unknown driver" error which is detected by `openMySQL()` (sentinel `errMySQLDriverUnavailable`); each MySQL importer then calls `logMySQLStub()` which writes the required warning log `"MySQL driver not available — migration would connect to {host}"` and returns nil so the job completes with 0 items. Adding `go get github.com/go-sql-driver/mysql` + a blank import `_ "github.com/go-sql-driver/mysql"` later activates real MySQL ingestion with no further code changes — the SQL queries (`SELECT ID, post_title, post_content, post_excerpt FROM wp_posts WHERE post_type = ? AND post_status IN (...)`) and per-entity write helpers are already in place.
- Created `/home/z/my-project/repos/lastsaas/backend/internal/api/handlers/migration_handler.go` (1331 lines) with:
  - **6 REST handlers**: `ListMigrations` (GET, filter by tenantId + optional platform/status, sort createdAt:-1, pagination), `GetMigration` (GET /{id}, single job + computed `progressPct`), `CreateMigration` (POST, validates platform enum, inserts MigrationJob with status=pending), `StartMigration` (POST /{id}/start, validates status ∈ {pending, failed}, transitions to running, emits `EventMigrationJobStarted`, launches `go h.runMigration(context.Background(), job)`, returns 202 Accepted with the running job), `CancelMigration` (POST /{id}/cancel, marks status=cancelled + completedAt, signals the goroutine via per-job cancel func), `GetMigrationLogs` (GET /{id}/logs, verifies job belongs to tenant, filters by tenantId+jobId+optional level, sort createdAt:1, pagination).
  - **Background dispatcher** `runMigration(parentCtx, job)`: wraps parent in a cancellable context, registers the cancel func in `h.cancels` (mutex-protected), dispatches to the platform-specific importer based on `job.Platform`, catches panics, honours `ctx.Err() == context.Canceled` (returns silently — CancelMigration already wrote the terminal state), and on normal return calls `completeJob()` or `failJob()`.
  - **Per-entity bookkeeping helpers**: `appendLog` (writes MigrationLog row), `recordMapping` (writes MigrationMapping row, treats duplicate-key as a warning since the importer is idempotent), `incrementMigrated` (atomic `$inc` on the appropriate migratedX field, mirrors in-memory job struct), `setTotals` (one round-trip `$set` of all 4 Total* counters), `emitBatchCompleted` (fires `EventMigrationBatchCompleted` with current counters), `insertCourse` (creates a Course with status=draft / priceType=free / currency=USD, de-duplicates slugs within the tenant by appending source ID).
  - **6 platform importers**: `migrateFromLearnDash` (queries `wp_posts` for post_type ∈ {sfwd-courses, sfwd-lessons, sfwd-quiz}), `migrateFromLifterLMS` (post_type ∈ {llms_course, llms_lesson, llms_quiz}), `migrateFromLearnPress` (post_type ∈ {lp_course, lp_lesson, lp_quiz}), `migrateFromWooCommerce` (products → courses, shop_order rows logged as enrollment stubs since student lookup by email isn't yet wired), `migrateFromCSV` (parses CSV at `sourceConfig.filePath`, header-required, expected columns title/description/instructor_email/price/category, tolerates ragged rows via `FieldsPerRecord=-1`), `migrateFromTutorLMS` (HTTP GET to `sourceConfig.apiUrl` with `Bearer {apiKey}` header, 30s timeout, decodes JSON array, gracefully degrades to warning log + 0 items on network/HTTP/JSON errors).
  - **Shared MySQL helpers**: `mysqlDSN` (builds `user:pass@tcp(host:port)/dbName?parseTime=true` from sourceConfig, defaults host=localhost port=3306, requires dbName), `openMySQL` (wraps `sql.Open` + `Ping`, detects "unknown driver" → sentinel), `querySourcePosts` (generic `SELECT ID, post_title, post_content, post_excerpt FROM wp_posts WHERE post_type = ? AND post_status IN (...)` — shared by all 4 WordPress importers), `importPostRows` (shared loop: inserts each row, writes log + mapping, increments counter, emits batch event every 10 items, skips empty titles with a warning, defers lesson/quiz target writes until course mappings are resolved).
- Cancellation design: `h.cancels map[primitive.ObjectID]context.CancelFunc` (mutex-protected). `StartMigration` → `runMigration` registers a per-job cancel func; `CancelMigration` first marks the DB row cancelled, then calls `cancelJobContext(id)` which invokes the registered func. Importer loops check `ctx.Err()` between every row and return `ctx.Err()` immediately; `runMigration` detects `ctx.Err() == context.Canceled` and returns WITHOUT calling `completeJob`/`failJob` so the cancelled status already written by the handler isn't overwritten.
- Reused `getLMSContext` / `respondWithJSON` / `respondWithError` / `decodeJSON` / `parsePositiveInt` / `slugify` helpers — no redeclaration. The new `MigrationHandler.ListMigrations`/`CreateMigration` methods do NOT collide with the legacy `LMSHandler.ListMigrations`/`LMSHandler.CreateMigration` stubs in `lms.go` line 3986-3987 (different receiver types: `*MigrationHandler` vs `*LMSHandler`).
- Ran `gofmt -w internal/api/handlers/migration_handler.go` to align struct field tags / struct-literal alignment with the existing codebase style.
- Verified with `go build ./...` from `/home/z/my-project/repos/lastsaas/backend` — exit code 0, zero output.
- Verified with `gofmt -l internal/api/handlers/migration_handler.go` — empty output (file is gofmt-clean).
- Verified with `go vet ./internal/api/handlers/...` — exit code 0. Pre-existing vet warnings in `tenant_test.go` (lines 61, 71 — "using resp before checking for errors") are unrelated to Phase 6 and were already documented in P6-A1's worklog entry.

Stage Summary:
- Created `migration_handler.go` (1331 lines) with 6 REST handlers + 21 private helper/platform functions.
- Build status: PASS (`go build ./...` exits 0, zero output).
- gofmt status: PASS (file is gofmt-clean).
- Routes: `GET /api/lms/migrations` (ListMigrations), `GET /api/lms/migrations/{id}` (GetMigration), `POST /api/lms/migrations` (CreateMigration), `POST /api/lms/migrations/{id}/start` (StartMigration, launches goroutine), `POST /api/lms/migrations/{id}/cancel` (CancelMigration, signals goroutine via per-job context), `GET /api/lms/migrations/{id}/logs` (GetMigrationLogs). Routes are NOT yet registered in the server's router (cmd/server/main.go or wherever the LMS subrouter is wired) — that wiring is out of scope for this task and should be done by the route-registration task.
- MySQL driver: STUB. `github.com/go-sql-driver/mysql` is not in `go.mod`; the 4 MySQL-backed importers (LearnDash / LifterLMS / LearnPress / WooCommerce) detect the missing driver at runtime via the "unknown driver" error from `sql.Open("mysql", dsn)` and fall back to logging the required warning `"MySQL driver not available — migration would connect to {host}"` and completing the job with 0 items. The SQL queries (`SELECT ID, post_title, post_content, post_excerpt FROM wp_posts WHERE post_type = ?`) and per-entity write helpers (`insertCourse`, `appendLog`, `recordMapping`, `incrementMigrated`, `emitBatchCompleted`) are fully implemented — adding `go get github.com/go-sql-driver/mysql` + a blank import is the only change required to activate real MySQL ingestion.
- CSV importer: REAL (no external deps; uses stdlib `encoding/csv`). Reads `sourceConfig.filePath`, requires a `title` column, tolerates ragged rows, de-duplicates slugs.
- TutorLMS importer: REAL (uses stdlib `net/http`). Makes a GET request to `sourceConfig.apiUrl` with `Bearer {apiKey}` header, decodes the JSON array, and gracefully degrades to a warning log + 0 items on any network / HTTP / JSON error.
- Events emitted: `EventMigrationJobStarted` (StartMigration), `EventMigrationJobCompleted` (runMigration on nil return), `EventMigrationJobFailed` (runMigration on error return or panic), `EventMigrationBatchCompleted` (every 10 entities). The 4 legacy `migration.*` events in `lms_events.go` are intentionally NOT emitted by this handler (it uses the richer `migration.job_*` / `migration.batch_completed` namespaces defined in `reportsai_events.go`).

Notes for downstream agents:
- The 6 routes are NOT registered in the server's router yet. The route-registration task should add `migrationHandler := handlers.NewMigrationHandler(database, emitter)` to the server bootstrap and wire the 6 endpoints under the `/api/lms` subrouter alongside the existing LMS routes. Suggested paths (matching the spec):
  - `r.HandleFunc("/migrations", migrationHandler.ListMigrations).Methods("GET")`
  - `r.HandleFunc("/migrations/{id}", migrationHandler.GetMigration).Methods("GET")`
  - `r.HandleFunc("/migrations", migrationHandler.CreateMigration).Methods("POST")`
  - `r.HandleFunc("/migrations/{id}/start", migrationHandler.StartMigration).Methods("POST")`
  - `r.HandleFunc("/migrations/{id}/cancel", migrationHandler.CancelMigration).Methods("POST")`
  - `r.HandleFunc("/migrations/{id}/logs", migrationHandler.GetMigrationLogs).Methods("GET")`
- `StartMigration` resets the per-run counters (`migratedCourses`/`migratedLessons`/`migratedQuizzes`/`migratedStudents` to 0) and clears `errorMessage` when re-running a failed job, but does NOT reset `totalX` counters — the importer's first step is `setTotals()` which overwrites them with fresh values from the source. If a re-run is attempted against a different source (e.g. different DB), the totals will be overwritten correctly.
- `importPostRows` writes courses immediately but DEFERS lesson/quiz target writes — it logs each extracted lesson/quiz as an info row and increments the migrated counter, but doesn't insert a `Lesson` / `Quiz` document because resolving the parent `courseId` requires looking up the `MigrationMapping` for the source course. A follow-up pass can iterate `MigrationMapping` rows where `entityType=course`, find each course's lessons in the source, and write them via `db.Lessons().InsertOne`. The structure (`importCourseRow`, `insertCourse`, the shared loop in `importPostRows`) is set up to make that follow-up straightforward.
- `MigrationMapping` has a unique compound index on `(tenantId, jobId, entityType, sourceId)` per P6-A1. `recordMapping` treats the resulting duplicate-key error as a warning (not a failure) so a re-run of the same job is idempotent — entities already mapped are simply re-logged as warnings and the importer moves on.
