...
const initialState: AuthContextType = {
  isAuthenticated: true, // DEV: bypass auth for course builder experiment
  isLoading: false,
  isInitialized: true,
  errorMessage: null,
  user: { id: "1", name: "tutor", email: "admin@tutor.hellotutorlms.com", username: "tutor", avatar: "" } as any,
  login: async () => {},
  logout: async () => {},
};
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    // DEV: skip init() that would overwrite our pre-authenticated state
    return;
    const init = async () => {
      try {
