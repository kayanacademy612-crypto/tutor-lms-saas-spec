...
class CourseComingSoon {
	...
	public function __construct() {
		...
		add_filter( 'posts_where', array( $this, 'exclude_future_bundles' ), 10, 2 );
	...
	/**
	 * Exclude bundles from course list that are future status.
	 *
	 * @since 4.0.0
	 *
	 ...
	 * @return string
	 */
	public function exclude_future_bundles( $where, $query ) {
		if ( $query->get( 'exclude_future_bundles' ) ) {
			global $wpdb;

			$where .= $wpdb->prepare( " AND NOT ( {$wpdb->posts}.post_type = %s AND {$wpdb->posts}.post_status = %s )", tutor()->bundle_post_type, 'future' );
		}
	...
	public function set_course_coming_soon_archive_query( $query ) {
		...
		$query->set( 'exclude_future_bundles', true );
	...
	public function show_filtered_coming_soon_courses( $args ) {
		...
		$args['exclude_future_bundles'] = true;
