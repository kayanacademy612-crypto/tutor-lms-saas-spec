...
class QuizImporter {
	...
	/**
	 * Question types that use mask file in answer_two_gap_match.
	 *
	 * @since 4.0.0
	 *
	 * @var string[]
	...
	private function prepare_quiz_questions_answers( $quiz_questions_answers, $from_course = false ) {
		...
		$question_types = array_keys( QuizModel::get_question_types() );
		...
		foreach ( $answers as $answer ) {
			...
			$answer = $this->prepare_mask_for_import( $answer );
	...
	/**
	 * Prepare draw/pin mask value for import.
	 *
	 * Converts exported data-URI mask into a new local quiz-images file URL.
	 *
	 * @since 4.0.0
	 *
	 * @param array $answer Answer data row.
	 *
	 * @return array
	 */
	private function prepare_mask_for_import( array $answer ): array {
		$question_type = isset( $answer['belongs_question_type'] ) ? str_replace( '-', '_', (string) $answer['belongs_question_type'] ) : '';
		if ( ! in_array( $question_type, self::MASK_QUESTION_TYPES, true ) ) {
			return $answer;
		}

		$mask = isset( $answer['answer_two_gap_match'] ) ? self::normalize_quiz_mask_value( (string) $answer['answer_two_gap_match'] ) : '';
		if ( '' === $mask ) {
		...
		$answer['answer_two_gap_match'] = is_string( $saved_mask ) && '' !== $saved_mask ? $saved_mask : $mask;
		return $answer;
	}
