...
class Enrollment_Expiry {
	...
	public function __construct() {
		...
		add_action( 'single_bundle_template_before_load', array( $this, 'cancel_expired_enrolment' ), 10, 1 );
	...
	public function cancel_expired_enrolment( $course_id, $user_id = null ) {
		...
		if ( is_array( $results ) && count( $results ) ) {
			...
			if ( count( $ids ) ) {
				...
				$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->posts} SET post_status='%s' WHERE ID IN ({$in_clause})", 'cancel' ) );
	...
	public function show_expires_info_not_enrolled( $course_id ) {
		...
		if ( ! $this->is_enabled() || EnrollmentModel::is_enrolled( $course_id, get_current_user_id() ) ) {
			return;
		}
	...
	public function show_expires_info( $course_id ) {
		...
		$enrolment = EnrollmentModel::is_enrolled( $course_id, get_current_user_id() );
	...
	public function ajax_store_enrollment_extend_date() {
		...
		if ( $this::store_custom_expiry_date( $course_id, $student_id, $extend_date ) ) {
			...
			if ( strtotime( $extend_date ) > strtotime( 'today' ) ) {
				...
				do_action( 'tutor_after_enrollment_extend', $enrollment, $course_id, $student_id );
			}
	...
	public function filter_the_status( $status, $enrollment ) {
		try {
			if ( $this::is_expired( $status, $enrollment->enrol_id ) && $this->is_enabled() ) {
