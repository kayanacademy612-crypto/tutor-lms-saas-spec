...
// ResumeSubscription clears the cancel-at-period-end flag on a Stripe
// subscription, effectively resuming it through the end of the current period.
// Used when a user reactivates a subscription they had previously scheduled
// for cancellation.
func (s *Service) ResumeSubscription(ctx context.Context, subscriptionID string) error {
	...
	apicounter.StripeAPICalls.Add(1)
	if err != nil {
		return fmt.Errorf("stripe resume subscription: %w", err)
	}
	return nil
}

// RetrySubscriptionPayment attempts to pay the latest open invoice for a
// Stripe subscription. Used during the dunning workflow when a renewal
// payment has failed and the user (or an admin) manually triggers a retry.
// Returns nil on success or a wrapped error describing the failure.
func (s *Service) RetrySubscriptionPayment(ctx context.Context, subscriptionID string) error {
	sub, err := s.GetSubscription(ctx, subscriptionID)
	if err != nil {
		return fmt.Errorf("stripe get subscription for retry: %w", err)
	}
	if sub.LatestInvoice == nil || sub.LatestInvoice.ID == "" {
		return fmt.Errorf("subscription %s has no latest invoice to retry", subscriptionID)
	}
	_, err = invoice.Pay(sub.LatestInvoice.ID, &stripe.InvoicePayParams{})
	apicounter.StripeAPICalls.Add(1)
	if err != nil {
		return fmt.Errorf("stripe pay invoice %s: %w", sub.LatestInvoice.ID, err)
	}
	return nil
}
